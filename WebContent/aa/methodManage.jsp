					...
					<c:forEach items="${rows }" var="ins" varStatus="vs">
						<tr>
							<td><input type="checkbox" onclick="onSelect(this.checked)"
								name="idlist" value="${ins.aaa901 }"></td>
							<td>${vs.count }</td>
							<td><a href="#" onclick="onEdit('${ins.aaa901 }')">${ins.aaa902 }</a>
							</td>
							<td>${ins.aaa903 }</td>
							<td>${ins.aaa904 }</td>
							<td>${ins.aaa905 }</td>
							...
							<td>${ins.aaa910 }</td>
							<td>${ins.aaa911 }</td>
							<td><a href="#" onClick="onDel('${ins.aaa901}')">删除</a></td>
						</tr>
					</c:forEach>
		...
		<table border="1" align="center" width="95%">
			<tr>
				<td align="center"><input type="submit" value="查询" name="next">
					<input type="submit" value="添加" name="next" formaction="<%=path %>/aa/methodAdd.jsp"> 
					<input type="submit" value="删除" name="next" disabled="disabled" formaction="<%=path %>/aa/aa09Delete.html" id="del">
				</td>
			</tr>
		</table>
